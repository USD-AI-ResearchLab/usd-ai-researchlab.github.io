    ...
    <div className="fixed top-0 left-0 right-0 bg-white/90 backdrop-blur-md border-b border-gray-200/50 h-20 flex items-center justify-between px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 z-50">
      ...
      <div className={`fixed top-20 right-0 w-64 min-h-screen bg-white/90 backdrop-blur-2xl flex flex-col items-start justify-start gap-6 pt-8 px-6 pb-8 md:hidden transition-all duration-500 ease-in-out shadow-2xl overflow-y-auto z-40 ${
        isOpen ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'
          ...
          <Link to="/about" onClick={toggleMenu}><div className="text-lg font-medium" style={{ color: 'inherit' }} onMouseEnter={(e) => (e.target as HTMLElement).style.color = 'var(--logo-red, #C53030)'} onMouseLeave={(e) => (e.target as HTMLElement).style.color = 'inherit'}>About</div></Link>
          <div className="w-full">
            ...
            {isMobilePeopleOpen && (
              <div className="ml-4 space-y-4">
                <Link to="/faculty" onClick={toggleMenu}><div className="text-base font-medium py-1" onMouseEnter={(e) => (e.target as HTMLElement).style.color = 'var(--logo-red, #C53030)'} onMouseLeave={(e) => (e.target as HTMLElement).style.color = 'inherit'}>Faculty</div></Link>
                <Link to="/staff" onClick={toggleMenu}><div className="text-base font-medium py-1" onMouseEnter={(e) => (e.target as HTMLElement).style.color = 'var(--logo-red, #C53030)'} onMouseLeave={(e) => (e.target as HTMLElement).style.color = 'inherit'}>PhD Students</div></Link>
                <Link to="/students" onClick={toggleMenu}><div className="text-base font-medium py-1" onMouseEnter={(e) => (e.target as HTMLElement).style.color = 'var(--logo-red, #C53030)'} onMouseLeave={(e) => (e.target as HTMLElement).style.color = 'inherit'}>Masters Students</div></Link>
              </div>
            )}
          </div>
